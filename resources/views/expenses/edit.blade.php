    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ تعديل مصروف</span>
            </div>
        </div>
...
@section('content')
    ...
    <form action="{{ route('expenses.update', $expense->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
        {{ method_field('patch') }}
        {{ csrf_field() }}
        ...
        <div class="row">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="Expense_name" name="Expense_name" 
                               value="{{ old('Expense_name', $expense->Expense_name) }}" required placeholder="ادخل اسم المصروف">
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="reference_number" name="reference_number" 
                                       value="{{ old('reference_number', $expense->reference_number) }}" placeholder="ادخل رقم المرجع">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control select2" id="payment_method" name="payment_method" required>
                                    ...
                                    <option value="bank" {{ old('payment_method', $expense->payment_method) == 'bank' ? 'selected' : '' }}>تحويل بنكي</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <label>
                                    <input type="checkbox" id="recurring" name="recurring" value="1" 
                                           {{ old('recurring', $expense->recurring) ? 'checked' : '' }}>
                                    مصروف متكرر
                                </label>
                            </div>
                        </div>
                        <div class="col-md-6" id="recurringTypeDiv" style="{{ old('recurring', $expense->recurring) ? '' : 'display:none;' }}">
                            <div class="form-group">
                                <label for="recurring_type">نوع التكرار</label>
                                <select class="form-control select2" id="recurring_type" name="recurring_type">
                                    <option value="daily" {{ old('recurring_type', $expense->recurring_type) == 'daily' ? 'selected' : '' }}>يومي</option>
                                    <option value="weekly" {{ old('recurring_type', $expense->recurring_type) == 'weekly' ? 'selected' : '' }}>اسبوعي</option>
                                    <option value="monthly" {{ old('recurring_type', $expense->recurring_type) == 'monthly' ? 'selected' : '' }}>شهري</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="4" 
                                  placeholder="ادخل وصف المصروف">{{ old('description', $expense->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-4">
                <div class="form-section">
                    ...
                    @if($expense->attachment)
                        <div class="alert alert-info mb-3">
                            <i class="fas fa-paperclip"></i> يوجد مرفق سابق
                            <br>
                            <a href="{{ asset('uploads/expenses/' . $expense->attachment) }}" target="_blank" class="btn btn-sm btn-primary mt-2">
                    ...
                    <div class="form-group">
                        <label for="attachment">{{ $expense->attachment ? 'استبدال الملف' : 'ارفاق ملف' }}</label>
                ...
                <div class="form-section">
                    <div class="form-section-title">معلومات الانشاء والتعديل</div>
                    <p class="text-muted">
                        <i class="fas fa-user"></i> انشأ بواسطة: <strong>{{ $expense->Created_by }}</strong>
                    </p>
                    <p class="text-muted">
                        <i class="fas fa-calendar-plus"></i> انشأ في: <strong>{{ $expense->created_at->format('Y-m-d H:i') }}</strong>
                    </p>
                    @if($expense->updated_at)
                        <p class="text-muted">
                            <i class="fas fa-edit"></i> عدل في: <strong>{{ $expense->updated_at->format('Y-m-d H:i') }}</strong>
                        </p>
                    @endif
                </div>
